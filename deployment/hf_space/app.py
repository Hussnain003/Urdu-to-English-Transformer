"""
Hugging Face Space: Urdu -> English translation demo.
Loads final_model.pt + sp_src.model/sp_tgt.model (bundled in this repo)
and serves a Gradio UI. Model/tokenizer code mirrors translate.py.
"""
import math
...
def load_model(checkpoint_path="final_model.pt"):
    ...
    model.eval()
    return model, ckpt.get("test_results", {})


model, test_results = load_model()

EXAMPLES = [
    "پبلک ٹرانسپورٹ ایک بدنامی ہے۔",
    "میڈم صدر ، ایک اور سنجیدہ معاملہ ہے جو منظر عام پر آیا۔",
    "کچھ کیڑے ، جیسے بیڈ بگ ، جلن ہیں۔",
]


def translate(text, beam_size):
    return translate_sentence(model, text, beam_size=int(beam_size))


metrics_line = (
    f"Test set — BLEU: {test_results.get('bleu', 0):.2f} | "
    f"chrF: {test_results.get('chrf', 0):.2f} | "
    f"TER: {test_results.get('ter', 0):.2f}"
)

with gr.Blocks(title="Urdu → English Translator") as demo:
    gr.Markdown("# Urdu → English Translator")
    gr.Markdown(
        "A from-scratch Transformer (52M params) trained on ~91K Urdu-English "
        "sentence pairs. Works best on general/news-style sentences; may struggle "
        "with proper nouns, rare vocabulary, and very long or technical sentences."
    )
    gr.Markdown(f"*{metrics_line}*")

    with gr.Row():
        src_box = gr.Textbox(label="Urdu input", placeholder="یہاں اردو جملہ لکھیں...", rtl=True, lines=3)
        out_box = gr.Textbox(label="English translation", lines=3, interactive=False)

    beam_slider = gr.Slider(minimum=1, maximum=8, value=4, step=1, label="Beam size")
    btn = gr.Button("Translate", variant="primary")

    btn.click(fn=translate, inputs=[src_box, beam_slider], outputs=out_box)
    src_box.submit(fn=translate, inputs=[src_box, beam_slider], outputs=out_box)

    gr.Examples(examples=EXAMPLES, inputs=src_box)
