"""
Urdu -> English translation served over HTTP, for Cloud Run.
Same model/tokenizer logic as translate.py and the HF Space app.py,
wrapped in FastAPI instead of a CLI prompt or Gradio UI.
"""
import math
import os
...
app = FastAPI(title="Urdu to English Translator")
...
@app.get("/health")
def health():
    return {"status": "ok", "test_results": test_results}


@app.post("/translate", response_model=TranslateResponse)
def translate(req: TranslateRequest):
    result = translate_sentence(model, req.text, beam_size=req.beam_size)
...
def home():
    ...
    <html><body style="font-family: sans-serif; max-width: 640px; margin: 40px auto;">
      <h2>Urdu &rarr; English Translator</h2>
      <textarea id="src" rows="3" style="width:100%; direction: rtl;" placeholder="یہاں اردو جملہ لکھیں..."></textarea>
      <br><br>
      <button onclick="run()">Translate</button>
      <p><b>Result:</b> <span id="out"></span></p>
      <script>
        async function run() {
          ...
          const res = await fetch('/translate', {
            ...
            body: JSON.stringify({text: text, beam_size: 4})
          });
          const data = await res.json();
          document.getElementById('out').innerText = data.translation;
        }
      </script>
    </body></html>
    """
